...
`SELECT eu.papel FROM empresa_usuarios eu WHERE eu.usuario_id = $1 AND eu.papel = 'ADMIN' AND eu.ativo = true LIMIT 1`,
[req.userId]
);
...
const result = await pool.query(
  `SELECT
   ...
   WHERE eu.empresa_id = $1 AND eu.ativo = true
   ORDER BY u.nome ASC`,
  [empresaId]
);
...
const novoUser = await client.query(
  ...
  [nome, email, senhaHash, senhaHash]
);
