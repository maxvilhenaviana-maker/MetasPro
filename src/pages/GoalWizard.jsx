...
// Design: painel de controle industrial, tipografia técnica, dark premium
...
const PRESSOES = [
  {
    value: 'MODERADO', pct: 0.25, label: 'Moderado',
    tag: '25%',
    desc: 'Meta atingível em 6 a 9 dos 12 períodos históricos.',
    color: C.green, dim: C.greenDim,
    icon: '▬▬░░',
  },
  {
    value: 'INTERMEDIARIO', pct: 0.50, label: 'Intermediário',
    tag: '50%',
    desc: 'Meta atingível em 3 a 6 dos 12 períodos históricos.',
    color: C.blueLight, dim: C.blueDim,
    icon: '▬▬▬░',
  },
  {
    value: 'DESAFIADOR', pct: 0.75, label: 'Desafiador',
    tag: '75%',
    desc: 'Meta atingível em 1 a 3 dos 12 períodos históricos.',
    color: C.amber, dim: C.amberDim,
    icon: '▬▬▬▬',
  },
  {
    value: 'ALAVANCADO', pct: 1.00, label: 'Alavancado',
    tag: '100%',
    desc: 'Meta atingível apenas no ápice do histórico registrado.',
...
function calcPreview(data, direcao, pressaoPct) {
  ...
  let intervalM, meta;
  if (direcao === 'AUMENTAR') {
    intervalM = Math.max(...nums) - mean;
    meta = mean + pressaoPct * intervalM;
  } else {
    intervalM = mean - Math.min(...nums);
    meta = mean - pressaoPct * intervalM;
  }
...
function Sidebar({ current }) {
  return (
    <div style={{
      ...
      <p style={{ margin: '0 0 24px', fontSize: 11, fontWeight: 700, color: C.textDim, letterSpacing: '0.12em', textTransform: 'uppercase' }}>
        Configuração
      </p>
      {STEPS.map((s, i) => {
          ...
          <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', position: 'relative' }}>
            ...
            <div>
              <p style={{
                ...
                color: done ? C.greenLight : active ? C.text : C.textDim,
...
function Step1({ data, onChange, unidades, loadingUnidades }) {
  return (
    <StepWrapper
      title="Identificação da Meta"
      subtitle="Nomeie e contextualize o indicador que será monitorado"
      icon="◎"
    >
      <Field label="Nome da Meta *" hint="Ex: Volume de Vendas — Loja Centro">
        <Input
          value={data.nome_meta}
          onChange={e => onChange('nome_meta', e.target.value)}
          placeholder="Descreva o indicador de forma clara e objetiva"
        />
      </Field>

      <Field label="Unidade Monitorada *" hint="Selecione a unidade responsável por esta meta">
        <Select
        ...
        >
          ...
          {unidades.map(u => (
            <option key={u.id} value={u.id}>
              {u.nome_unidade} {u.codigo_unidade ? `(${u.codigo_unidade})` : ''}
            </option>
          ))}
        </Select>
      </Field>
      ...
      <Field label="Descrição do Objetivo" hint="Contexto estratégico ou operacional (opcional)">
        <Textarea
          value={data.objetivo_descritivo}
          onChange={e => onChange('objetivo_descritivo', e.target.value)}
          placeholder="Ex: Aumentar a participação de mercado no segmento premium..."
          rows={3}
        />
      </Field>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        <Field label="Apresentação do Resultado">
  ...
  const opts = [
    {
      ...
      emoji: '📈',
      title: 'Aumentar',
      desc: 'O resultado deve crescer ao longo do tempo.',
      examples: 'Vendas, Receita, Produção, NPS, Clientes',
      color: C.green, dim: C.greenDim, border: C.green,
    },
    {
      value: 'REDUZIR',
      emoji: '📉',
      title: 'Reduzir',
      desc: 'O resultado deve diminuir ao longo do tempo.',
      examples: 'Custos, Rejeições, Tempo de espera, Devoluções',
      color: C.blueLight, dim: C.blueDim, border: C.blueLight,
    },
  ];

  return (
    <StepWrapper
      title="Direção da Meta"
      subtitle="Defina se o objetivo é crescer ou reduzir o indicador"
      icon="⇅"
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginTop: 8 }}>
        {opts.map(opt => {
          ...
          return (
            <button
              key={opt.value}
              onClick={() => onChange('direcao', opt.value)}
              style={{
            ...
            >
              ...
              <div>
                ...
                <p style={{ margin: 0, fontSize: 11, color: C.textDim }}>
                  Exemplos: <span style={{ color: active ? opt.color : C.textDim }}>{opt.examples}</span>
                </p>
              </div>
              <div style={{
      ...
      {data.direcao && (
        <InfoBox color={data.direcao === 'AUMENTAR' ? C.green : C.blueLight}>
          A IA irá identificar o{' '}
          <strong>{data.direcao === 'AUMENTAR' ? 'maior' : 'menor'}</strong>{' '}
          valor da amostra limpa para calcular o Intervalo M e sugerir a meta ideal.
        </InfoBox>
      )}
    </StepWrapper>
  );
...
function Step3({ data, onChange }) {
  return (
    <StepWrapper
      title="Periodicidade"
      subtitle="Com que frequência os resultados serão registrados e controlados?"
      icon="⟳"
    >
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 8 }}>
        {PERIODICIDADES.map(p => {
          ...
          return (
            <button
              key={p.value}
              onClick={() => onChange('periodicidade_resultado', p.value)}
              style={{
            ...
            >
              <p style={{
                ...
                color: active ? C.blueLight : C.text,
                fontFamily: "'Sora', sans-serif",
              }}>{p.label}</p>
      ...
      {data.periodicidade_resultado === 'PERSONALIZADA' && (
        <div style={{ marginTop: 16 }}>
          <Field label="Descreva a periodicidade personalizada">
            <Input
              value={data.periodicidade_custom || ''}
              onChange={e => onChange('periodicidade_custom', e.target.value)}
              placeholder="Ex: A cada 45 dias, por safra, por campanha..."
            />
          </Field>
        </div>
      )}
      ...
      {data.periodicidade_resultado && (
        <InfoBox color={C.blueLight}>
          Você registrará resultados <strong>{PERIODICIDADES.find(p => p.value === data.periodicidade_resultado)?.label?.toLowerCase()}</strong>.
          O sistema alertará quando o prazo de registro se aproximar.
        </InfoBox>
      )}
    </StepWrapper>
  );
...
function Step4({ data, onChange }) {
  ...
  return (
    <StepWrapper
      ...
      subtitle="Define o grau de desafio da meta em relação ao histórico"
      icon="⚡"
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 8 }}>
        {PRESSOES.map(p => {
          ...
          return (
            <button
              key={p.value}
              onClick={() => onChange('nivel_pressao', p.value)}
              style={{
      ...
      {selected && (
        <div style={{
          ...
          background: `linear-gradient(135deg, ${selected.dim}, transparent)`,
...
function Step5({ data, onChange }) {
  ...
  const handleChange = (i, val) => {
    const next = [...vals];
    next[i] = val;
    onChange('historico', next);
  };

  return (
    <StepWrapper
      title="Resultados Históricos"
      subtitle={`Insira os últimos resultados de ${periodLabel.toLowerCase()}. Mínimo 6, ideal 12 períodos.`}
      icon="∿"
    >
      {/* Grid de inputs */}
          ...
          return (
            <div key={i} style={{ position: 'relative' }}>
              <label style={{
                ...
                color: hasVal ? C.blueLight : C.textDim,
              ...
              <input
                ...
                style={{
                  ...
                  background: hasVal ? 'rgba(37,99,235,0.08)' : C.surface,
                  border: `1.5px solid ${hasVal ? C.blue : C.border}`,
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 12 }}>
        <div style={{ flex: 1, height: 4, background: C.border, borderRadius: 4, overflow: 'hidden' }}>
          <div style={{
            ...
            width: `${(filled.length / 12) * 100}%`,
            background: filled.length >= 6 ? C.green : C.amber,
          }} />
        </div>
        <span style={{
          fontSize: 12, fontWeight: 700, color: filled.length >= 6 ? C.greenLight : C.amber,
      ...
      {preview && (
        <div style={{
          ...
          <div style={{
            padding: '10px 16px', background: 'rgba(37,99,235,0.08)',
            borderBottom: `1px solid ${C.border}`,
          }}>
            <p style={{ margin: 0, fontSize: 11, fontWeight: 700, color: C.blueLight, letterSpacing: '0.08em' }}>
              ⚡ PRÉ-VISUALIZAÇÃO (sem IA — dados brutos)
            </p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 0 }}>
            ...
            <PreviewCell
              label="Meta estimada"
              value={fmt(preview.meta)}
              highlight
              color={pressao?.color}
            />
          </div>
          <p style={{ margin: 0, padding: '8px 16px 10px', fontSize: 11, color: C.textDim }}>
            * A IA irá remover outliers e refinar este cálculo na próxima etapa.
          </p>
        </div>
      )}
      ...
      {filled.length < 6 && filled.length > 0 && (
        <InfoBox color={C.amber}>
          Preencha pelo menos <strong>6 períodos</strong> para uma análise estatística confiável.
        </InfoBox>
      )}
    </StepWrapper>
  );
...
// ─── STEP 6: Revisão ──────────────────────────────────────────────────────────
function Step6({ data, unidades, onSubmit, loading, error }) {
  const pressao  = PRESSOES.find(p => p.value === data.nivel_pressao);
  ...
  return (
    <StepWrapper
      title="Revisão Final"
      subtitle="Confirme as configurações antes de enviar para análise da IA"
      icon="◈"
    >
      <div style={{
        ...
        {rows.map((r, i) => (
          <div key={i} style={{
            ...
            <span style={{ fontSize: 13, fontWeight: 600, color: C.text, textAlign: 'right', maxWidth: '55%' }}>
              {r.value}
            </span>
          </div>
        ))}
      ...
      <div style={{
        ...
        <p style={{ margin: 0, fontSize: 12, color: C.greenLight, lineHeight: 1.5 }}>
          🤖 Ao confirmar, a <strong>IA analisará</strong> os dados históricos, identificará outliers,
          calculará a média limpa e sugerirá a meta final com justificativa.
        </p>
      </div>
...
// ─── Tela de Resultado ────────────────────────────────────────────────────────
function ResultScreen({ result, onNew, onDashboard }) {
    ...
    <div style={{ padding: '32px 36px', animation: 'fadeIn 0.5s ease' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: 28 }}>
        <div style={{
          ...
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: 28, boxShadow: '0 0 0 10px rgba(22,163,74,0.12)',
      ...
      {result.outliersExcluidos?.length > 0 && (
        <div style={{
          ...
          <p style={{ margin: '0 0 6px', fontSize: 12, fontWeight: 700, color: C.amber }}>
            🔍 Outliers removidos pela IA
          </p>
          <p style={{ margin: '0 0 8px', fontSize: 13, color: '#fde68a', fontFamily: "'Fira Code', monospace" }}>
      ...
      <div style={{
        ...
        <p style={{ margin: '0 0 6px', fontSize: 12, fontWeight: 700, color: C.blueLight }}>
          💡 Por que esta meta?
        </p>
        <p style={{ margin: 0, fontSize: 13, color: C.textMd, lineHeight: 1.6 }}>
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        <button onClick={onNew} style={{
          ...
          border: `1px solid ${C.border}`, borderRadius: 12,
          color: C.textMd, fontSize: 14, fontWeight: 600, cursor: 'pointer',
          fontFamily: "'Sora', sans-serif",
        }}>
          + Nova Meta
        </button>
    ...
    <div style={{ padding: '32px 36px', animation: 'fadeIn 0.35s ease' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, marginBottom: 28 }}>
        <div style={{
          ...
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: 20, color: C.blueLight, fontFamily: "'Fira Code', monospace",
        }}>
          {icon}
        </div>
        <div>
          <h2 style={{ margin: '0 0 4px', fontSize: 20, fontWeight: 800, color: C.text, fontFamily: "'Sora', sans-serif" }}>
            {title}
          </h2>
...
function Field({ label, hint, children }) {
  return (
    <div>
      <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: C.textDim, marginBottom: 6, letterSpacing: '0.07em', textTransform: 'uppercase' }}>
        {label}
      </label>
      {children}
  ...
  return (
    <input
      ...
      style={{
        ...
        background: focused ? 'rgba(37,99,235,0.06)' : C.surface,
  ...
  return (
    <select
      ...
      style={{
        ...
        borderRadius: 10, color: value ? C.text : C.textDim,
  ...
  return (
    <textarea
      ...
      style={{
        ...
        border: `1.5px solid ${focused ? C.blue : C.border}`,
        borderRadius: 10, color: C.text, fontSize: 14, outline: 'none',
        transition: 'all 0.2s', resize: 'vertical', lineHeight: 1.5,
...
function InfoBox({ color, children }) {
  return (
    <div style={{
      ...
      borderRadius: 10, fontSize: 12, color: C.textMd, lineHeight: 1.5,
    }}>
      {children}
    </div>
  );
...
function PreviewCell({ label, value, highlight, color, border }) {
  return (
    <div style={{
      ...
      <p style={{ margin: '0 0 4px', fontSize: 10, color: C.textDim, letterSpacing: '0.06em', textTransform: 'uppercase' }}>
        {label}
      </p>
      <p style={{
...
function ResultCard({ label, value, highlight, color }) {
  return (
    <div style={{
      ...
      <p style={{ margin: '0 0 4px', fontSize: 10, color: highlight ? color : C.textDim, letterSpacing: '0.06em', textTransform: 'uppercase' }}>
        {label}
      </p>
      <p style={{
...
const INITIAL = {
  ...
  apresentacao: 'NUMERO',
  direcao: 'AUMENTAR',
  periodicidade_resultado: 'MENSAL',
  periodicidade_custom: '',
  nivel_pressao: 'MODERADO',
...
export default function GoalWizard() {
  ...
  const canProceed = () => {
    ...
    if (step === 5) {
      const filled = form.historico.filter(v => v !== '' && !isNaN(Number(v)));
  ...
  const handleNew = () => { setForm(INITIAL); setStep(1); setResult(null); setError(''); };
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {result ? (
              ...
              <>
                ...
                {step === 6 && <Step6 data={form} unidades={unidades} onSubmit={handleSubmit} loading={loading} error={error} />}

                {/* Navegação */}
                <div style={{
                  display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                  padding: '0 36px 32px', gap: 12,
                }}>
                  <button
                    onClick={back} disabled={step === 1}
                    style={{
                      padding: '11px 24px',
                      background: 'transparent', border: `1px solid ${step === 1 ? C.border : C.borderHi}`,
                      borderRadius: 10, color: step === 1 ? C.textDim : C.textMd,
                  ...
                  <span style={{ fontSize: 11, color: C.textDim, fontFamily: "'Fira Code', monospace" }}>
                    {step} / {STEPS.length}
                  </span>
                  ...
                  {step < 6 && (
                    <button
                      ...
                      style={{
                        ...
                        boxShadow: canProceed() ? '0 4px 16px rgba(37,99,235,0.25)' : 'none',
                      }}
                    >
                      Continuar →
                    </button>
